...
#数据部分
#汇总部分及数据
#测试时间  提交代码commit id  测试数据集  测试结果  失败原因
# test_result_data={"commit_times_id":commit_times_id,"测试时间":'{date}'.format(date=time.strftime('%Y%m%d%H%M%S')),"提交代码commit_id":"get_commit_id",  "测试数据集":"test_pic_set",  "测试结果":"result_staus" }

#设定头显示类型及字段
# 测试时间	算法版本	测试图片集	解码率	正确率	耗时	最大耗时	最小耗时	扫码失败总数
# 2021-8-18-7-9-36	v0.10.3h	/mnt/2d/DM码采集/	100.00%	69.40%	121.6555288	777	7	887
# test_cases={"reslut_set_id":reslut_set_id,"测试时间":s["date_time"],"算法版本":s["version"],	"测试图片集":s["dataset_path"],"解码率":decoding_rate,"正确率":decoding_accuracy,"耗时":"run_time_avg","最大耗时":"run_time_max","最小耗时":"run_time_min","扫码失败总数":succeed_false_count,"扫码失败文件分布":"扫码失败文件分布"}

#详情界面的标题，数据

# image_path	image_name	succeed	run_time	type	result	target_result	rect_points
# test_data={"image_path":str(s["decode_results"][i]["image_path"]),	"image_name":str(s["decode_results"][i]["image_path"]).split("/")[-1],	"succeed":s["decode_results"][i]["succeed"],"run_time":"run_time",	"type":"type",	"result":"result","target_result":"target_result","rect_points":"rect_points"}
...
if __name__ == '__main__':
    ...
    commit_times_id=1
    #for i in range(len(s["dataset_path"])):
    ...
    #总表数据
    table_td = html.TABLE_TMPL_TOTAL % dict(commit_times_id=test_result_data["commit_times_id"],测试时间=test_result_data['测试时间'],提交代码commit_id=test_result_data['提交代码commit_id'],测试数据集=test_result_data['测试数据集'],测试结果 = test_result_data['测试结果'])
    table_tr0 += table_td
    commit_times_id+=1
       # i+=1
    reslut_set_id=1
    ...
    table_td_module=html.TABLE_TMPL_MODULE % dict(reslut_set_id=test_cases["reslut_set_id"],测试时间=test_cases["测试时间"],算法版本=test_cases["算法版本"],测试图片集=test_cases["测试图片集"],解码率=test_cases["解码率"],正确率=test_cases["正确率"],耗时=test_cases["耗时"],最大耗时=test_cases["最大耗时"],最小耗时=test_cases["最小耗时"],扫码失败总数=test_cases["扫码失败总数"],扫码失败文件分布=test_cases["扫码失败文件分布"])
    ...
    for i in range(len(s["decode_results"])):
        ...
        table_td_case=html.TABLE_TMPL_CASE % dict(id=test_data["id"],image_path=test_data["image_path"],image_name=test_data["image_name"],succeed=test_data["succeed"],run_time=test_data["run_time"],type=test_data["type"],result=test_data["result"],target_result=test_data["target_result"],rect_points=test_data["rect_points"])
